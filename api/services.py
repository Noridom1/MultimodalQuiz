...
class NotebookService:
    ...
    def generate_quiz(
        ...
        *,
        ...
        try:
            # Temporary operational mode: disable image generation provider calls.
            # Questions are still generated by the LLM and can have no associated image.
            effective_mock_image = True
            # region agent log
            with open("debug-c02dfd.log", "a", encoding="utf-8") as _f:
                _f.write(json.dumps({
                    "sessionId": "c02dfd",
                    "runId": str(pending_run.get("run_id") or "pre-fix"),
                    "hypothesisId": "H2",
                    "location": "api/services.py:198",
                    "message": "generate_quiz backend received request",
                    "data": {
                        "source_id": source_id,
                        "num_questions": num_questions,
                        "question_format_distribution": question_format_distribution,
                        "effective_mock_image": effective_mock_image,
                    },
    ...
    def _build_run_summary(
        ...
        *,
        ...
        generated_title = self._generate_quiz_title(quiz_package, source_title=source_title, notebook_title=notebook_title)
        first_image = next(
    ...
    def _client_question_metadata(self, question: dict[str, Any]) -> dict[str, Any]:
        """Expose only frontend-safe metadata needed for interactive answer widgets."""
